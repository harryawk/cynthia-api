...
app.post('/postrequest', (req, res) => {
  ...
  the_request({
    ...
    var results = []
    var fs = require('fs')
    fs.writeFile(__dirname + '/public/' + '0' + '.jpg', 'data:image/jpeg;base64,' + result.result[0], function(err) {
      if (err) console.error(err)
      console.log('File saved')
      results.push('http://f072a7ca.ngrok.io/static/' + '0' + '.jpg')
      console.log('0')
      // if (x == result.result.length - 1) 
      fs.writeFile(__dirname + '/public/' + '1' + '.jpg', 'data:image/jpeg;base64,' + result.result[1], function(err) {
        if (err) console.error(err)
        console.log('File saved')
